...
@php
    $baseClasses = 'sidebar-link group flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200';

    if ($active) {
        $stateClasses = 'bg-emerald-600 text-white';
    } elseif ($danger) {
        $stateClasses = 'text-red-400 hover:bg-white/5';
    } else {
        $stateClasses = 'text-gray-300 hover:bg-white/5 hover:text-white';
    }
@endphp

<a href="{{ $href }}" data-tooltip="{{ $label }}" {{ $attributes->merge(['class' => "$baseClasses $stateClasses"]) }}>
