...
<div class="sales-page">
    ...
    <div class="flex items-start justify-between mb-6">
        <div>
            ...
            <p class="text-gray-500 mt-1">Automatically tracked from completed orders for better decision-making.</p>
        </div>
        <div class="flex items-center gap-3">
            <button class="border border-gray-300 hover:bg-gray-50 text-gray-700 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
            ...
            <button class="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 shadow-sm shadow-emerald-200">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {{-- Today's Sales --}}
        <div class="bg-white border border-gray-200 rounded-xl p-5 text-white">
            <div class="flex items-center justify-between mb-3">
                ...
                <span class="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full bg-white/20 text-[10px] font-semibold">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            @endphp
            <p class="text-2xl font-bold text-gray-900 mt-0.5">₱{{ number_format($weeklyTotal) }}</p>
            <p class="text-gray-400 text-[10px] mt-1">{{ $weeklyCount }} transactions</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        <div class="flex items-center justify-between mb-2">
            <div>
                ...
                <p class="text-xs text-gray-400 mt-0.5">Daily revenue · Last 7 days</p>
            </div>
            <div class="flex items-center gap-2">
                <button class="px-3 py-1.5 text-xs font-medium rounded-lg bg-emerald-50 text-emerald-600 border border-emerald-200">Daily</button>
                <button class="px-3 py-1.5 text-xs font-medium rounded-lg text-gray-500 hover:bg-gray-50 border border-gray-200">Weekly</button>
        ...
        <div class="relative">
            <svg viewBox="0 0 660 200" class="w-full h-auto" style="overflow: visible;" preserveAspectRatio="xMidYMid meet">
                ...
                @foreach($dPoints as $i => $p)
                    @php
                        $stW = 64; $stH = 20;
                        $stX = max($stW/2, min($p['x'], $dW - $stW/2));
                        $stY = max($p['y'] - 26, 2);
                        $stArrowX = $p['x'] - $stX;
                    @endphp
                    <g class="chart-dot-group" style="cursor: pointer;">
                        ...
                        <g class="chart-tooltip" opacity="0" transform="translate({{ $stX }}, {{ $stY }})">
                            <rect x="{{ -$stW/2 }}" y="-2" width="{{ $stW }}" height="{{ $stH }}" rx="5" fill="#1f2937" opacity="0.95"/>
                            <polygon points="{{ $stArrowX - 3 }},{{ $stH - 2 }} {{ $stArrowX + 3 }},{{ $stH - 2 }} {{ $stArrowX }},{{ $stH + 2 }}" fill="#1f2937" opacity="0.95"/>
                            <text x="0" y="11" text-anchor="middle" fill="white" font-size="7" font-weight="600">{{ $p['label'] }}: ₱{{ number_format($p['amount']) }}</text>
                        </g>
                    </g>
                @endforeach
            </svg>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <div class="bg-green-50 rounded-xl px-4 py-3 border border-green-100 flex items-center gap-3">
            ...
            <div>
                <p class="text-xs text-green-700 font-semibold">{{ $totalTransactions ?? 0 }} Completed</p>
                <p class="text-[10px] text-green-600/70">All time orders</p>
            </div>
        </div>
        <div class="bg-blue-50 rounded-xl px-4 py-3 border border-blue-100 flex items-center gap-3">
            <div class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            </div>
            <div>
                <p class="text-xs text-blue-700 font-semibold">Top: {{ $topCategoryName ?? 'N/A' }}</p>
                <p class="text-[10px] text-blue-600/70">Best selling category</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-0">
        <div class="flex flex-wrap items-center gap-3">
            <div class="flex-1 min-w-[200px] relative">
                ...
                <input type="text" placeholder="Search by transaction ID, customer name..." class="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-gray-50">
            </div>
            <select class="border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-gray-50">
            ...
            <input type="date" class="border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-gray-50">
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-emerald-50/30 transition-colors group">
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button class="w-7 h-7 rounded-lg hover:bg-gray-100 flex items-center justify-center transition-colors" title="View Details">
                ...
                <tr>
                    <td colspan="7" class="px-5 py-12 text-center text-gray-400">
                        <svg class="w-12 h-12 mx-auto text-gray-300 mb-3" fill="none" stroke="currentColor" stroke-width="1" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5m16.5 0h-16.5"/></svg>
                        <p class="text-sm font-medium">No completed sales yet</p>
                        <p class="text-xs mt-1">Sales will appear here when orders are completed</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
        ...
        <div class="flex items-center justify-between px-5 py-4 border-t border-gray-100 bg-gray-50/50">
            <p class="text-xs text-gray-500">Showing <span class="font-semibold text-gray-700">{{ $salesPaginated->firstItem() }}-{{ $salesPaginated->lastItem() }}</span> of <span class="font-semibold text-gray-700">{{ $salesPaginated->total() }}</span> transactions</p>
            <div class="flex items-center gap-1">
                ...
                @foreach($salesPaginated->getUrlRange(1, $salesPaginated->lastPage()) as $page => $url)
                    ...
                    <button class="w-8 h-8 text-xs font-semibold text-white bg-emerald-600 rounded-lg">{{ $page }}</button>
                    @else
