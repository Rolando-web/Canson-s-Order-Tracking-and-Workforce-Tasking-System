    ...
    <div class="mt-6 bg-white rounded-xl border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-4">
            <div>
                ...
                <p class="text-sm text-gray-500">Completed orders ready to be assigned to an employee for delivery</p>
            </div>
            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-purple-100 text-purple-700">{{ count($readyForDelivery) }} orders</span>
        </div>

        @if(count($readyForDelivery) === 0)
        <div class="text-center py-10">
            <svg class="w-12 h-12 text-gray-300 mx-auto mb-3" fill="none" stroke="currentColor" stroke-width="1" viewBox="0 0 24 24">
            ...
            <p class="text-gray-500 font-medium">No orders pending delivery</p>
            <p class="text-gray-400 text-sm mt-1">Completed orders will appear here when ready for delivery</p>
        </div>
        @else
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4" id="readyForDeliveryGrid">
            @foreach($readyForDelivery as $order)
            <div class="border-2 border-gray-200 rounded-xl p-4 hover:border-purple-400 hover:shadow-md transition-all group" data-dispatch-id="{{ $order['dispatch_id'] }}">
                {{-- Order Header --}}
                <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center gap-2">
                    ...
                    <span class="inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold bg-purple-50 text-purple-600 border border-purple-200">READY</span>
                </div>
                ...
                {{-- Footer --}}
                <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                    <p class="text-sm font-bold text-purple-600">₱{{ number_format((float)$order['total_amount'], 2) }}</p>
                    <button onclick="openAssignDeliveryModal({{ json_encode($order) }})" class="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-purple-700 bg-purple-50 border border-purple-200 rounded-lg hover:bg-purple-100 transition-colors opacity-0 group-hover:opacity-100">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 016.75 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z"/></svg>
                        Assign Delivery
                    </button>
                </div>
            </div>
